...
const chkEx=(ex,w)=>{ if(!ex||!ex.type) return E(w+': missing type');
 ...
 if(T==='fill') ex.items.forEach((it,i)=>{ const a=[].concat(it.answer); a.forEach(x=>{ if(ex.bank.indexOf(x)<0) E(`${w} item${i}: answer ${x} not in bank`);}); const n=(it.q.match(/＿＿/g)||[]).length; if(n!==a.length) E(`${w} item${i}: ${n} blanks but ${a.length} answers`);});
 ...
 if(T==='listening') ex.items.forEach((it,i)=>{ if(i===0&&!it.script) E(w+': first item needs script'); if(!(it.answer>=0&&it.answer<it.options.length)) E(`${w} item${i}: bad answer`);});
};
const nos=[];
...
strs.forEach(s=>{ const t=s.replace(/\{[^{}|]+\|[^{}|]+\}/g,''); if(/[{}]/.test(t)) E('bad ruby/braces: '+s.slice(0,80)); if(/\{[^{}|]*[ぁ-ん]+\|/.test(s)&&/\{[ぁ-ん]+\|/.test(s)) E('ruby base is kana: '+s.slice(0,60)); if((s.match(/\*\*/g)||[]).length%2) E('unbalanced **: '+s.slice(0,80)); });
